    ...
    <>
      <nav className="fixed top-0 left-0 p-4 py-2 pt-4 w-full grid grid-cols-4 items-center mix-blend-exclusion max-md:flex max-md:justify-between max-md:items-center z-200">
        <div className="flex justify-start max-md:hidden">
          <Link
            to="/"
            className=" text-[.9em] max-lg:text-[.95em] max-md:text-[1em]  text-s font-medium tracking-[-0.03em] truncate"
          >
            T—ML
          </Link>
        </div>
        ...
        <div className="flex justify-start gap-1 max-md:hidden">
          <Link
            to="/works"
            className=" text-[.9em] max-lg:text-[.95em] max-md:text-[1em]   text-s font-medium tracking-[-0.03em]"
          >
            Work,
          </Link>
          <Link
        ...
        <div className="flex justify-center">
          {isWorkDetail && project ? (
            <div className="w-full flex items-center gap-1">
              <span className=" text-[.9em] max-lg:text-[.95em] max-md:text-[1em]  text-s font-semibold tracking-[-0.03em]">
                {project.title}
              </span>
              <span className=" text-[.9em] max-lg:text-[.95em] max-md:text-[1em]  text-s font-semibold tracking-[-0.15em]">
                —
              </span>
              <button
                onClick={() => setProjectModal(true)}
                className=" text-[.9em] max-lg:text-[.95em] max-md:text-[1em] text-s font-semibold tracking-[-0.03em] cursor-pointer"
              >
                Information
              </button>
            </div>
